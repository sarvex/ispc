...
do
    ...
    $KW_SERVER_PATH/bin/kwadmin load --url https://$KW_SERVER:$KW_SERVER_PORT --force --name $build_name $KW_PROJECT_NAME $GITHUB_WORKSPACE/release/kw_tables
    if [ $? -eq 0 ]
    then
        break
    fi
...
echo "$build_name" > $GITHUB_WORKSPACE/kw_build_number
