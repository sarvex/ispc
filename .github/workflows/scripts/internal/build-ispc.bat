...
if defined signing_required (
    ...
    signtool.exe sign /fd sha256 /sha1 %SIGNING_HASH% /tr http://timestamp.comodoca.com/rfc3161 /td sha256 bin/Release/ispcrt.dll || goto :error
)

REM Package ISPC
cmake --build . --target PACKAGE --config Release || goto :error

REM Sign ISPC package
if defined signing_required (
    signtool.exe sign /fd sha256 /sha1 %SIGNING_HASH% /tr http://timestamp.comodoca.com/rfc3161 /td sha256 *.msi || goto :error
)
