...
for i in range(0,3):
    if exists[i]:
        print_debug(take_lines(f"{names[i]} --version", "first"), False, "")
    else:
        error(f"you don't have {names[i]}", 0)
if exists[0] and exists[1] and exists[2] and common.check_tools(2):
    print_debug("Tools' versions are ok\n", False, "")
print_debug("\nSDE:\n", False, "")
if exists[3]:
    print_debug(take_lines(f"{names[3]} --version", "first"), False, "")
else:
    error(f"you don't have {names[3]}", 2)
print_debug("\nISPC:\n", False, "")
if exists[4]:
    print_debug(take_lines(f"{names[4]} --version", "first"), False, "")
else:
    error(f"you don't have {names[4]}", 2)
print_debug("\nC/C++ compilers:\n", False, "")
for i in range(5,8):
    if exists[i]:
        print_debug(take_lines(f"{names[i]} --version", "first"), False, "")
    else:
        error(f"you don't have {names[i]}", 2)
print_debug("\nCMake:\n", False, "")
if exists[8]:
    first_line = take_lines(f"{names[8]} --version", "first")
    matched_version = re.search(r"\d+\.\d+\.\d+", first_line)
    if matched_version is None:
        error("Unable to parse cmake version")
    else:
        cmake_version = matched_version[0]
...
if os.environ.get("SDE_HOME") is None:
